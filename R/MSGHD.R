...
mainMSGHD<-function(data=NULL, gpar0, G, n, label  , eps, method ,nr=NULL ) {
   ...
   #   if(min(label)==0&max(label)==G){
      ...
      if (is.null(gpar0)) gpar  = rgparMS(data=data, g=G, w=z,l=lc)
      else gpar  = gpar0
    ...
    val = list(loglik= temp[[1]], gpar=temp[[2]], par=par,z=z, map=MAPMS(data=data, gpar= temp[[2]], label=label), BIC=BIC,ICL=ICL,AIC=AIC,AIC3=AIC3)
    return(val)
}
...
#    val=list(BIC=BIC,model=model)
    val=MixGHD(Index=BIC,AIC=model$AIC,AIC3=model$AIC3,BIC=model$BIC,ICL=model$ICL, map=model$map, gpar=model$gpar,par=model$par, loglik=model$loglik, z=model$z,  method="MSGHD",data=as.data.frame(data),scale=scale)
...
#        val=list(ICL=ICL,model=model)
        val=MixGHD(Index=ICL,AIC=model$AIC,AIC3=model$AIC3,BIC=model$BIC,ICL=model$ICL, map=model$map, gpar=model$gpar,par=model$par, loglik=model$loglik, z=model$z, method="MSGHD",data=as.data.frame(data),scale=scale)
...
#        val=list(AIC3=AIC3,model=model)
        val=MixGHD(Index=AIC3,AIC=model$AIC,AIC3=model$AIC3,BIC=model$BIC,ICL=model$ICL, map=model$map, gpar=model$gpar,par=model$par, loglik=model$loglik, z=model$z,  method="MSGHD",data=as.data.frame(data),scale=scale)
...
#        val=list(AIC=AIC,model=model)
        val=MixGHD(Index=AIC,AIC=model$AIC,AIC3=model$AIC3,BIC=model$BIC,ICL=model$ICL, map=model$map, gpar=model$gpar,par=model$par, loglik=model$loglik, z=model$z,  method="MSGHD",data=as.data.frame(data),scale=scale)
