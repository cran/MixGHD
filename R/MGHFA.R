...
MGHFA<- function(data=NULL, gpar0=NULL, G=2, n=100, label =NULL  ,q=2,epsilon=1e-2 ) {
...
##n number of iterations
	...
	data=scale(data)
	if (is.null(gpar0)) gpar = igparM(data=data, g=G,q=q)
	else gpar  = gpar0
	loglik = numeric(n)
	for (i in 1:3) {
	...
	pcol=ncol(data)
	BIC=2*loglik[n]-log(nrow(data))*((G-1)+G*(3*pcol+2+pcol*q))
